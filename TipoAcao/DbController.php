...
use App\Models\TipoAcaoModels;
# use App\Models\VTabelaPrincipalModelsModels;
use Exception;

class TipoAcaoDbController extends BaseController
{
    // private $ModelUpload;
    private $ModelTipoAcao;
    ...
    {
        $this->ModelTipoAcao = new TipoAcaoModels();
    ...
    public function dbFields($processRequestFields = array())
    {
        ...
        $dbCreate = array();
        $autoColumn = $this->ModelTipoAcao->getColumnsFromTable();
    ...
    public function dbCreate($parameter = NULL)
    {
        ...
        try {
            $this->ModelTipoAcao->dbCreate($this->dbFields($parameter));
            $affectedRows = $this->ModelTipoAcao->affectedRows();
            if ($affectedRows > 0) {
                $dbCreate['insertID'] = $this->ModelTipoAcao->insertID();
    ...
    public function dbUpdate($key, $parameter = NULL)
    {
        if (
            ...
            $this->ModelTipoAcao->dbUpdate($key, $parameter);
        } else {
            $this->ModelTipoAcao->dbUpdate($key, $this->dbFields($parameter));
        }
        try {
            $affectedRows = $this->ModelTipoAcao->affectedRows();
            if ($affectedRows > 0) {
    ...
    {
        try {
            $this->ModelTipoAcao->dbDelete('id', $parameter);
            $affectedRows = $this->ModelTipoAcao->affectedRows();
            if ($affectedRows > 0) {

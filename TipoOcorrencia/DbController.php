...
use App\Models\AgendamentoTipoModel;
...
class DbController extends BaseController
{
    ...
    {
        $this->ModelAgendamentoTipo = new AgendamentoTipoModel();
        // $this->ModelUpload = new UploadModel();
        // $this->ModelVTabelaPrincipal = new VTabelaPrincipalModelsModels();
        $this->uri = new \CodeIgniter\HTTP\URI(current_url());
        $this->pagination = new System\SBaseController();
        $this->message = new System\MessageController();
    }
    ...
    public function dbFields($processRequestFields = array())
    {
        ...
        $dbCreate = array();
        $autoColumn = $this->ModelAgendamentoTipo->getColumnsFromTable();
    ...
    public function dbCreate($parameter = NULL)
    {
        ...
        try {
            $this->ModelAgendamentoTipo->dbCreate($this->dbFields($parameter));
            $affectedRows = $this->ModelAgendamentoTipo->affectedRows();
            if ($affectedRows > 0) {
                $dbCreate['insertID'] = $this->ModelAgendamentoTipo->insertID();
    ...
    public function dbUpdate($key, $parameter = NULL)
    {
        if (
            ...
            $this->ModelAgendamentoTipo->dbUpdate($key, $parameter);
        } else {
            $this->ModelAgendamentoTipo->dbUpdate($key, $this->dbFields($parameter));
        }
        try {
            $affectedRows = $this->ModelAgendamentoTipo->affectedRows();
            if ($affectedRows > 0) {
    ...
    {
        try {
            $this->ModelAgendamentoTipo->dbDelete('id', $parameter);
            $affectedRows = $this->ModelAgendamentoTipo->affectedRows();
            if ($affectedRows > 0) {

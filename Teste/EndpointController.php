...
class EndpointController extends ResourceController
{
    ...
    # Consumo de API
    # route GET /www/index.php/novo/usuario/endpoint/sessao_cookie/(:any)
    # route POST /www/index.php/novo/usuario/endpoint/sessao_cookie/(:any)
    # Informação sobre o controller
    # retorno do controller [VIEW]
    public function testeSessao($parameter = NULL)
    {
        ...
        $montaJSON['tokenCsrf'] = $this->tokenCsrf->token_csrf();
        #
        $montaJSON['getMethod'] = $getMethod;
        $montaJSON['varSession'] = (session()->get('gov_br')) ? (session()->get('gov_br')) : (array());

        $cookieValue = $this->request->getCookie('gov_br_data');

        if ($cookieValue) {
            $montaJSON['cookieValue'] = json_decode($cookieValue, true);
        }
